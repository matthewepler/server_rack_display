...
function createBalls() {
	...
	// Ram Use  = number
	var numBalls = floor( map(ramUse, 0, 1, 0, width/20));
	if (balls.length < numBalls) {
		for (var i=0; i< numBalls - balls.length; i++) {
			balls.push( new Ball(palettes[palette][floor( random(0, 5))]));
		}
	...
	// CPU Load = speed
	var baseSpeed = floor( map(cpuLoad, 0, 1, 0.5, 50));
	for (var i=0; i<balls.length; i++) {
		if( baseSpeed > 0) {
...
function resetBallsPos() {
	for (var i=0; i<balls.length; i++) {
		balls[i].position = createVector(random(0, width), random(0, height));
	}
